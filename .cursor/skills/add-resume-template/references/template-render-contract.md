...
| `rt-title` | 模块标题 | 识别标题内容；允许直接跨页裁切 |
| `rt-item` | 一条教育/项目/经历 | 识别逻辑条目和内容底部；允许跨页 |
| `rt-desc` | 条目长描述 | 识别长描述与真实内容范围 |
| `rt-text` | 普通长文本 | 识别普通长文本与真实内容范围 |
| `rt-preserve-text` | 用户输入的描述/列表 | `pre-wrap`、强制长串换行 |
| `rt-list` | 荣誉/证书列表 | 识别列表内容与真实内容范围 |
| `rt-skills` / `rt-skill` | 技能容器/标签 | 接收皮肤与字体设置 |
| `rt-label` | 基本信息标签 | 接收标签字体色 |
| `rt-value` | 基本信息值 | 接收基本信息内容色 |
| `rt-name` | 顶栏大姓名 | 接收姓名颜色和统一字号比例 |

分页器会读取 `.rt-item`、`.rt-text`、`.rt-desc` 等语义内容和全部可见文本节点，用于确定真实内容底部、过滤纯尾部空白。断点本身固定为每页可用高度的整数倍，不根据标题、条目或段落回退。

模板不得添加 `break-before`、`break-after`、`break-inside: avoid`、`page-break-*` 或自定义空占位来干预编辑器分页。标题、条目和长描述都应接受直接跨页；需要微调落点时使用已有字体与间距设置。
...
3. 模块内标题：`.rt-title`、`h2`、`h3`。
4. 条目：`.rt-item`，以及旧模板中 `rt-desc` 的直接条目父级。
5. 完整文档内每个可见文本节点的 Range 行矩形。

分页策略：

1. 有效页高为 `A4 高度 - pageTopGap - pageBottomGap`。
2. 断点固定为 `有效页高 × 页码索引`，不因标题、条目或文字行提前回退。
3. 每个非末页的 viewport 使用完整有效页高，模块和条目允许直接跨页。
4. 只用真实内容底部过滤纯尾部空白和幽灵页。
5. PDF只克隆屏幕已经渲染的 `.preview-page`，不重新计算或重排内部内容。
...
- 把多条经历拼成一个 `rt-item`，导致条目语义、点击范围和内容识别混乱。
- 只用背景图承载正文、标题或字段值。
