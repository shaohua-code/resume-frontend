...
import Steps, { Step } from 'ant-design-vue/es/steps'
import {
  FileSearchOutlined,
  ThunderboltOutlined,
  EditOutlined,
...
<template>
  <Teleport to="body">
    ...
    >
      ...
      <div class="relative z-10 flex max-h-[min(90dvh,720px)] w-full max-w-xl flex-col overflow-hidden rounded-banner bg-surface shadow-float">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
          ...
          <!-- 指引会与路由切换并行显示，步骤组件局部导入以消除全局注册竞态。 -->
          <Steps :current="currentStep" size="small" class="mb-6">
            <Step v-for="item in steps" :key="item.title" :title="item.shortTitle" />
          </Steps>
          <section class="flex items-start gap-4 rounded-card border border-line/70 bg-gradient-to-br from-canvas/90 to-surface p-4 sm:items-center sm:p-6">
            <div
            ...
            <div class="min-w-0">
              ...
              <p class="mt-3 rounded-button bg-surface/75 px-3 py-2 text-xs leading-5 text-brand-dark">
